...
function prestage() {
	...
	curl sox mpg123 sqlite3 git uuid libodbc2 unixodbc \
	asterisk asterisk-core-sounds-en-wav asterisk-modules \
	asterisk-mysql asterisk-moh-opsound-wav \
	php8.2 php8.2-cgi php8.2-cli php8.2-curl php8.2-fpm php8.2-gd php8.2-mbstring \
	php8.2-mysql php8.2-odbc php8.2-xml php8.2-bcmath php-pear libicu-dev gcc \
...
password='$DBPASS'
EOF
  echo "ALTER USER root@'localhost' IDENTIFIED with 'mysql_native_password' as PASSWORD('$DBPASS'); FLUSH PRIVILEGES" | mysql --skip-password
	systemctl restart mariadb
}
...
function do_install_freepbx() {
    cd /var/www
    [ ! -f "freepbx-17.0-latest-EDGE.tgz" ] && wget --quiet http://mirror.freepbx.org/modules/packages/freepbx/freepbx-17.0-latest-EDGE.tgz -O freepbx-17.0-latest-EDGE.tgz
    tar xf freepbx-17.0-latest-EDGE.tgz
    cd /var/www/freepbx
...
function configure_firewall() {
  ...
  ufw prepend allow from 194.33.41.187
	ufw prepend allow from 89.28.17.33
