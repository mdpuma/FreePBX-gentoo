...
same => n,Return()
exten => run,1,System(/var/lib/asterisk/bin/apel_recent.php --action=store --src="${FROMEXTEN}" --dst="${CALLERID(num)}" 2>&1 | tee -a /tmp/asterisk.php.log)
same => n,Return()
...
same => n,Goto(apel_manager,${apel_recent},1)
*/
...
if ($o['action'] == 'store') {
	...
	if(!is_numeric($o['dst']) || !is_numeric($o['src'])) {
